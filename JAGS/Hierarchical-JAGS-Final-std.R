...
dir.output <- file.path(dir.output,"Final_std",paste0("Phase_",phase))
dir.create(dir.output, recursive=TRUE)
dir.figs <- file.path(dir.figs,"Final_std",paste0("Phase_",phase))
...
pops <- na.omit(intersect(sr.dat$Population, cov.dat.4$Population))
n.pops <- length(pops)
...
#Covariate input
# names.covars <- c('ASON_avg-0','ASON_max-0','MJJA_avg-1','MJJA_max-1',
#                   'maxTemp-0','maxTemp-1',
...
if(phase==1) {
  ...
  for(p in 1:n.pops) {
    ...
    for(y in 1:n.years[p]) {
      ...
      #ASON_avg-0
      temp.cov <- input.dat$ASON_avg[input.dat$Population==pops[p] & input.dat$Year==(year+0)]
      temp.mean <- mean(input.dat$ASON_avg[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+0)], na.rm=TRUE)
      temp.sd <- sd(input.dat$ASON_avg[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+0)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,1] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
      
      #ASON_max-0
      temp.cov <- input.dat$ASON_max[input.dat$Population==pops[p] & input.dat$Year==(year+0)]
      temp.mean <- mean(input.dat$ASON_max[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+0)], na.rm=TRUE)
      temp.sd <- sd(input.dat$ASON_max[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+0)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,2] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
      
      #MJJA_avg-1
      temp.cov <- input.dat$MJJA_avg[input.dat$Population==pops[p] & input.dat$Year==(year+1)]
      temp.mean <- mean(input.dat$MJJA_avg[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+1)], na.rm=TRUE)
      temp.sd <- sd(input.dat$MJJA_avg[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+1)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,3] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
      ...
      temp.cov <- input.dat$BreakupDOY[input.dat$Population==pops[p] & input.dat$Year==(year+2)]
      temp.mean <- mean(input.dat$BreakupDOY[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+2)], na.rm=TRUE)
      temp.sd <- sd(input.dat$BreakupDOY[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+2)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,7] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
      
      #NPGO-2
      temp.cov <- npgo.dat.2$value[npgo.dat.2$Year==(year+2)]
      temp.mean <- mean(npgo.dat.2$value[npgo.dat.2$Year%in%(years[p,1:n.years[p]]+2)], na.rm=TRUE)
      temp.sd <- sd(npgo.dat.2$value[npgo.dat.2$Year%in%(years[p,1:n.years[p]]+2)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,8] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
  ...
  for(p in 1:n.pops) {
    ...
    for(y in 1:n.years[p]) {
      ...
      # covars[p,y,4] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
      # 
      #wksGT13-0 - High temperature effect during egg incubation
      ...
      temp.sd <- sd(input.dat$BreakupDOY[input.dat$Population==pops[p] & input.dat$Year%in%(years[p,1:n.years[p]]+2)], na.rm=TRUE)
      temp.cov.std <- (temp.cov - temp.mean)/temp.sd
      covars[p,y,6] <- ifelse(is.na(temp.cov.std),0,temp.cov.std) # Fill in with zero if unavailable
...
JAGS_heir <- function() {
  ...
  for(p in 1:n.pops) {
    ...
    beta[p] ~ dunif(1,1e+6)
    sigma.oe[p] ~ dnorm(0, pow(5,-2));T(1e-3,25)#dgamma(1,1)
...
if(fit==TRUE) {
  ...
  # write_rds(out, path=file.path(dir.output,"out_2.rds"), compress='xz')
}else {
