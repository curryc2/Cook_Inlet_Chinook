...
dir.output <- file.path(dir.output,"Final_New_ADFG",paste0("Phase_",phase))
dir.create(dir.output, recursive=TRUE)
dir.figs <- file.path(dir.figs,"Final_New_ADFG",paste0("Phase_",phase))
...
if(phase==5) {
  ...
  for(p in 1:n.pops) {
    ...
    for(y in 1:n.years[p]) {
      ...
      #QUADRATIC maxWkJA-0
      temp.cov <- (input.dat$std.maxWkJA[input.dat$Population==pops[p] & input.dat$Year==(year+0)])^2
      covars[p,y,1] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
      
      #QUADRATIC meanWKJJA-1
      temp.cov <- (input.dat$std.meanWkJJA[input.dat$Population==pops[p] & input.dat$Year==(year+1)])^2
      covars[p,y,2] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
...
JAGS_heir <- function() {
  ...
  for(p in 1:n.pops) {
    ...
    beta[p] ~ dunif(0,1)
    sigma.oe[p] ~ dnorm(0, pow(2,-2));T(1e-3,25)#dgamma(1,1)
  ...
  for(p in 1:n.pops) {
    for(y in 1:n.years[p]) {
      ...
      base.rec[p,y] <- spawn[p,y]*exp(alpha[p]-spawn[p,y]*beta[p])
...
InitFn = function() {
  ...
  Return <- list(beta=beta, obs.sigma=obs.sigma)
  return(Return)
}
...
plot(x=spawn[p,], y=rec[p,], pch=21, bg=rgb(0,0,1,alpha=0.5), ylim=c(0, max(rec[,p],apply(pred.rec[,p,1:n.years[p]], 2, median))))
