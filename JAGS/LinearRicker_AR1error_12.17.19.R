...
#Date: 6.10.18
#
# Modified by Erik Schoen, UAF on 4-8-19
# 
#==================================================================================================
# NOTES: Updated by ES to read in a single input data file including corrected
# SR data (as of 4.8.2019) and environmental covariates that are already 
# indexed to brood year and standardized. Old lines of code that
# joined the SR and covariate data, applied time lags, and standardized 
...
# setwd(wd)
dir.output.root <- file.path(wd,"Output", "LinearRicker_AR1error_12.17.19")
dir.figs.root <- file.path(wd,"Plots", "LinearRicker_AR1error_12.17.19")
...
# dat <- data %>% select(c("Population","BroodYear","EscapementMethod","Spawners","CoreRecruits","CoreRecruitsPerSpawner")) %>% filter(!is.na(Spawners), !is.na(CoreRecruits), !is.na(CoreRecruitsPerSpawner))
...
if(plot.SR==TRUE) {
  ...
  g3 <- ggplot(dat, aes(x=Spawners/1e3, y=log(CoreRecruitsPerSpawner), color=BroodYear)) +
    theme_bw() +
    geom_point( alpha=0.75) +
    scale_color_viridis() +
    facet_wrap(~Population, scales='free', ncol = 3) +
...
# 
# # temp.dat.alt <- temp.dat.2 %>% group_by(Population) %>%  mutate('std.wksGT13'=(wksGT13 - mean(wksGT13 , na.rm=TRUE))/sd(wksGT13, na.rm=TRUE),
# #                                     'std.maxWkJA'=(maxWkJA - mean(maxWkJA , na.rm=TRUE))/sd(maxWkJA, na.rm=TRUE),
# #                                     'std.wksGT15'=(wksGT15 - mean(wksGT15 , na.rm=TRUE))/sd(wksGT15, na.rm=TRUE),
# #                                     'std.meanWkJJA'=(meanWkJJA - mean(meanWkJJA , na.rm=TRUE))/sd(meanWkJJA, na.rm=TRUE))
# 
...
# intersect(cov.dat$Population, dat$Population)
# length(intersect(cov.dat$Population, dat$Population))

# setdiff(cov.dat.3$perName, dat$Population) #In covar not in sr
# setdiff(dat$Population, cov.dat.3$perName) #In sr not in covar

# dat <- cov.dat[!is.na(cov.dat$Population),]

pops <- sort(unique(dat$Population))
# pops <- na.omit(intersect(dat$Population, cov.dat$Population))
n.pops <- length(pops)
...
# correlation <- dat %>%  filter(Population %in% pops) %>% select(std.ASON_avg, std.ASON_max,
...
# cor.input <- dat %>% select(c('std.wksGT13','std.wksGT15',
...
dir.create(dir.output, recursive=TRUE)
dir.figs <- file.path(dir.figs.root,paste0("Phase_",phase))
dir.create(dir.figs, recursive=TRUE)
...
# TODO: Can we create the array of covariate values directly from the dat dataframe using dplyr,
# instead of nested loops?
# covars.A.df <- dat %>%
#   select(maxT_spawn.std.all,
#          maxP_spawn.std,
#          avgP_grow.std,
#          medianQ_grow.std,
#          RB_spawn.std,
...
#          breakup.std,
#          NPGO.std)
# covars.A <- as.array(covars.A.df)

# TODO: test whether covars.A is identical to covars. Might need to complete or expand to fill
...
for(p in 1:n.pops) {
  ...
  for(y in 1:n.years[p]) {
    ...
    # maxT_spawn
    temp.cov <- dat$maxT_spawn.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
    
    # avgT_rear
    temp.cov <- dat$avgT_rear.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
    ...
    #RB_spawn
    temp.cov <- dat$RB_spawn.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
    
    #RB_emerge
    temp.cov <- dat$RB_emerge.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
    ...
    #breakup
    temp.cov <- dat$breakup.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
    
    #NPGO
    temp.cov <- dat$NPGO.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
...
JAGS_heir <- function() {
  ...
  for(p in 1:n.pops) {
    ...
    for(y in 2:n.years[p]) {
      ...
      resid[p,y-1] <- ln.rec[p,y-1] - pred.rec[p,y-1]
      
      pred.rec[p,y] <- spawn[p,y]*exp(alpha[p] - spawn[p,y]*beta[p] + sum(cov.eff[p,y,1:n.covars]))
    }#next y
  ...
  for(p in 1:n.pops) {
    ...
    # First Year
    ln.rec[p,1] ~ dnorm(log(pred.rec[p,1]), pow(sigma.oe[p],-2))
    
    for(y in 2:n.years[p]) {
      # ln.rec[p,y] ~ dnorm(log(pred.rec[p,y]), pow(sigma.oe[p],-2))
      ln.rec[p,y] ~ dnorm(log(pred.rec[p,y]) + phi[p]*resid[p,y-1], pow(sigma.oe[p],-2))
...
for(p in 1:n.pops) {
  ...
  for(y in 1:n.years[p]) {
    ...
    # avgT_grow
    temp.cov <- dat$avgT_grow.std[dat$Population==pops[p] & dat$BroodYear==year]
    covars[p,y,c] <- ifelse(is.na(temp.cov),0,temp.cov) # Fill in with zero if unavailable
    c <- c+1
