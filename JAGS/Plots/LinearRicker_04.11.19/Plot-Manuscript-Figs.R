...
# Plot Group Means ================================================================
mu.list <- cbind(out.3$BUGSoutput$sims.list$mu.coef[,1:2], # wksGT13, wksGT15
...
par(mfrow=c(3,4), mar=c(5,1,1,0), oma=c(1,1,3,1))
c <- 1
for(c in 1:n.plot) {
  if(c %in% c(3:4)) {
...
par(mfrow=c(3,4), mar=c(2,5,3,1), oma=c(3,3,1,1))
c <- 1
for(c in 1:n.plot) {
